...
## tests from all liberty podman repos
waitForServerStart()
{
   local cid=$1
   local count=${2:-1}
   local end=$((SECONDS+120))
   while (( $SECONDS < $end && $(podman inspect -f {{.State.Running}} "${cid}") == "true" ))
   do
      local result=$(podman logs "${cid}" 2>&1 | grep "CWWKF0011I" | wc -l)
      if [ "${result}" = "${count}" ]
      then
         return 0
      fi
...
waitForServerStop()
{
   ...
   while (( $SECONDS < $end ))
   do
      local result=$(podman logs "${cid}" 2>&1 | grep "CWWKE0036I" | wc -l)
      if [ $result = 1 ]
      then
         return 0
      fi
...
testLibertyStopsAndRestarts()
{
    ...
    if [ "$1" == "OpenShift" ]; then
        ...
        local cid=$(podman run -d -u 1001:0 $IMAGE)
    else
        local cid=$(podman run -d $IMAGE)
    fi
    ...
    local pass
    podman logs "${cid}" 2>&1 | grep "ERROR" \
        || pass="passed"; true
